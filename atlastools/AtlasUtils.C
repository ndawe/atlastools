...
void myBoxText(Double_t x,Double_t y,Int_t mcolor,char *text,Style_t fstyle=1001)
{
 TLatex* l=new TLatex(x,y,text);
 l->SetTextAlign(12); //l.SetTextSize(tsize);
 l->SetNDC();

 Double_t tsize=l->GetYsize();
 Double_t boxsize=l->GetXsize();

 Double_t y1=y-0.65*tsize;
 Double_t y2=y+0.65*tsize;
 Double_t x1=x-0.15*tsize;
 Double_t x2=x1+0.3*tsize+boxsize;

 //printf("x1= %f x2= %f y1= %f y2= %f xs= %f ys= %f\n",x1,x2,y1,y2,l->GetXsize(),l->GetYsize());
